...
void movePersons() {
  for (Person p : persons) {
    ...
    if(home)
    pos = map.getScreenPosition(new Location(p.home.x, p.home.y));
    
    else
    //get the screen position for the relevant lat and long
    pos = map.getScreenPosition(new Location(p.targetMap.x, p.targetMap.y));
    
    p.arrive(new PVector(pos.x, pos.y));
    p.update();
    p.display();
  }
}
